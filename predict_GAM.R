...
predict_GAM <- function(species, years, plot.pdf = TRUE, save = FALSE){
  ...
  if(!file.exists("Results/gam.results.all.gamma3.RData")){
  ...
  for(j in 1:length(species)){
    ...
    if(plot.pdf){pdf(paste0("Results/GAMplotsMunic_", species[j], ".pdf"))}
